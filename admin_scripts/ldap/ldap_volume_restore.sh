#!/usr/bin/env sh
# Восстановление OpenLDAP из tar.gz (созданного ldap_volume_backup.sh).
...
stop_using_containers() {
	...
	if [ "$skip_stop" = "1" ]; then
		echo "Ошибка: том $vol занят работающими контейнерами. Остановите LDAP или уберите --skip_stop=1." >&2
		exit 1
	fi
