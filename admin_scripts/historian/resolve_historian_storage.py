#!/usr/bin/env python3
"""Читает смонтированный путь данных PostgreSQL (историческая БД) из docker compose config (JSON).
...
Аргументы: compose_file [compose_project_name] [postgres_service]
...
# Каталог данных кластера PostgreSQL в официальном образе postgres
POSTGRES_DATA_MOUNT = "/var/lib/postgresql/data"
MARKER = f":{POSTGRES_DATA_MOUNT}"
...
def main() -> int:
    if len(sys.argv) < 2:
        print(
            "usage: resolve_historian_storage.py COMPOSE_FILE [PROJECT] [SERVICE]",
            file=sys.stderr,
        )
        return 2
    compose_file = sys.argv[1]
    project_flag = sys.argv[2] if len(sys.argv) > 2 else ""
    service = sys.argv[3] if len(sys.argv) > 3 else "psql"
    ...
    try:
        ...
        print(f"resolve_historian_storage: docker compose config failed: {e}", file=sys.stderr)
        return 1
    except FileNotFoundError:
        print("resolve_historian_storage: docker not found in PATH", file=sys.stderr)
        return 1
    ...
    if service not in services:
        print(
            f"resolve_historian_storage: service {service!r} not in compose config",
            file=sys.stderr,
        )
        return 1
    ...
    for entry in vols:
        ...
        if not tgt or os.path.normpath(tgt) != os.path.normpath(POSTGRES_DATA_MOUNT):
            continue
        if not src:
            continue
    ...
    print(
        f"resolve_historian_storage: no mount for {POSTGRES_DATA_MOUNT!r} "
        f"on service {service!r}",
        file=sys.stderr,
    )
    return 1
...
def parse_volume_entry(entry: object) -> tuple[str | None, str | None]:
    ...
    if isinstance(entry, str):
        if MARKER in entry:
            ...
            return src, POSTGRES_DATA_MOUNT
        parts = entry.split(":")
        ...
        if len(parts) >= 2:
            tgt = parts[-1]
            src = ":".join(parts[:-1])
            if tgt == POSTGRES_DATA_MOUNT or os.path.normpath(
                "/" + tgt.lstrip("/")
            ) == os.path.normpath(POSTGRES_DATA_MOUNT):
                return src, POSTGRES_DATA_MOUNT
    return None, None
