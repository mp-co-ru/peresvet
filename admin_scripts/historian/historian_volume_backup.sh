...
#
# По умолчанию архивы пишутся в backups/historian (от корня репозитория; см. --backup_dir).
#
# Запуск только из корня репозитория (как admin_scripts/ldap/ldap_volume_backup.sh).
...
mkdir -p "$backup_dir_abs"
echo "Каталог бэкапов: $backup_dir_abs"
