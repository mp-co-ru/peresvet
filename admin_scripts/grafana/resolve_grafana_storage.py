#!/usr/bin/env python3
"""Читает смонтированный путь данных Grafana из docker compose config (JSON).
...
Аргументы: compose_file [compose_project_name] [grafana_service]
...
GRAFANA_DATA_MOUNT = "/var/lib/grafana"
MARKER = f":{GRAFANA_DATA_MOUNT}"
...
def main() -> int:
    if len(sys.argv) < 2:
        print(
            "usage: resolve_grafana_storage.py COMPOSE_FILE [PROJECT] [SERVICE]",
            file=sys.stderr,
        )
        return 2
    compose_file = sys.argv[1]
    project_flag = sys.argv[2] if len(sys.argv) > 2 else ""
    service = sys.argv[3] if len(sys.argv) > 3 else "grafana"
    ...
    try:
        ...
        print(f"resolve_grafana_storage: docker compose config failed: {e}", file=sys.stderr)
        return 1
    except FileNotFoundError:
        print("resolve_grafana_storage: docker not found in PATH", file=sys.stderr)
        return 1
    ...
    if service not in services:
        print(
            f"resolve_grafana_storage: service {service!r} not in compose config",
            file=sys.stderr,
        )
        return 1
    ...
    for entry in vols:
        ...
        if not tgt or os.path.normpath(tgt) != os.path.normpath(GRAFANA_DATA_MOUNT):
            continue
        if not src:
            continue
    ...
    print(
        f"resolve_grafana_storage: no mount for {GRAFANA_DATA_MOUNT!r} "
...
def parse_volume_entry(entry: object) -> tuple[str | None, str | None]:
    ...
    if isinstance(entry, str):
        ...
        if len(parts) >= 2:
            tgt = parts[-1]
            src = ":".join(parts[:-1])
            if tgt == GRAFANA_DATA_MOUNT or os.path.normpath(
                "/" + tgt.lstrip("/")
            ) == os.path.normpath(GRAFANA_DATA_MOUNT):
                return src, GRAFANA_DATA_MOUNT
    return None, None
