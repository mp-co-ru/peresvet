...
Для WebSocket-сценариев, где обычного ``Request`` нет, hook передаёт объект
соединения в поле ``connection`` и технические данные в ``environment``.
Например, связь коннектора по ``/v1/connectors/{connector_id}`` вызывает
действие ``prsConnector.connector_ws`` и передаёт заголовки WebSocket-запроса.
