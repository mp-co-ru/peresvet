...
Тревоги в конфигураторе
^^^^^^^^^^^^^^^^^^^^^^^^^

Тревога создаётся **внутри тега** (синяя кнопка с колокольчиком / сущность
тревога). В панели свойств задаётся JSON-конфигурация: тип (порог, задержка по
диапазону, метод), пороги, длительность в микросекундах, автоквитирование,
идентификатор дочернего метода для сложной тревоги.

Полное описание логики тревог, полей ``prsJsonConfigString``, событий в AMQP и
работы с API см. в :ref:`alerts_guide`.
