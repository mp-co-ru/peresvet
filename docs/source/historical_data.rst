...
.. list-table:: Коды качества
   ...
   * - 101
     - Установлена (или восстановлена) связь между коннектором и платформой
       после состояния 100. Используется совместно со значением тэга ``null``.
   * - 102
   ...
   * - 104
     - Зарезервировано (ранее использовалось как «восстановлена связь»;
       больше не применяется в новых версиях платформы).
      ...
      "data": [
        {
          ...
          "tagId": "id тега",
          "data": [
            ["2018-12-31T00:00:00+03:00", 1, null],
            ["2018-12-31T00:01:00+03:00", 120, null]
          ]
        }
      ]
   }
...
.. note::
  В более ранних версиях платформы (и в некоторых старых примерах) порядок мог встречаться как
  ``[y, x, q]``. Актуальный контракт — **всегда** ``[x, y, q]``.
...
#. Связь с коннектором восстановлена после кода 100. Платформа записывает в
   привязанные теги значение ``null``, код качества ``q`` = 101 (до ответа
   коннектору полной конфигурацией; см. ``connectors.rst``).
