...
Кроме того, реализованы дополнительные режимы тревог (поле ``kind`` в
``prsJsonConfigString``), подробно описанные в :ref:`alerts_guide`:

* **Задержка по диапазону** (``kind: "duration"``): тревога возникает, если
  значение тега непрерывно остаётся в заданном диапазоне не меньше заданного
...
**Например:**
Тревога должна возникнуть, если температура пара в паропроводе находится в
диапазоне от 200 |deg| C до 230 |deg| C не менее трёх минут (значение
...
* **Сложные тревоги** (``kind: "complex"``): к тревоге привязан дочерний
  вычислительный ``prsMethod`` с инициаторами (теги, расписания); метод по RPC
  возвращает признак необходимости тревоги. Позволяет учитывать несколько тегов
  и произвольную логику.
...
Коннектор устанавливается как можно ближе к источнику данных и связывается с
платформой по протоколу MQTT (рекомендуемый способ; мост RabbitMQ MQTT plugin).
Исторически поддерживается также связь по WebSocket. Коннектор передаёт в
платформу считанные из источника данные и принимает от платформы сообщения об
изменениях параметров тегов и конфигурации.

Платформа отслеживает наличие связи с коннекторами и записывает в привязанные
теги коды качества при потере/восстановлении связи и при старте/остановке
платформы. Подробнее см. :doc:`Коннекторы <connectors>` (раздел «Связь по MQTT и коды качества тегов»).
