...
.. warning:: На рисунке видно, что ``objects_api_crud`` может принимать
   сообщения от пользователя и из очереди.
...
1. При старте сервис ``Сущность_2`` уведомляет сервис ``Сущность_1`` о том,
   что перед удалением или изменением узлов ``Сущность_1`` его, то есть
   сервис ``Сущность_2``, необходимо уведомить.
2. Сервис ``Сущность_1`` хранит список заинтересованных сервисов.
3. Перед изменением или удалением "своего" узла сервис
   ...
   * ``deleting``: в случае, если на предыдущее сообщение все заинтересованные
     ...
     этого сообщения все заинтересованные сервисы (``Сущность_2``) корректно
     удаляют "свои" узлы из иерархии и отсылают ответ сущности ``Сущность_1``
