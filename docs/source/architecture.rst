...
.. list-table:: Сообщения, генерируемые сервисом ``<сущность>_api_crud``
   :header-rows: 1
   ...
   * - action
     ...
     - Какому сервису предназначено
   * - create
     - <сущность>_api_crud_publish
     - `+`
     - request:
       ...
       .. code:: python

          {
            "action": "create",
            "data": {
               # идентификатор родительского узла или None
               "parentId": "<some_id or None>",
               # атрибуты cоздаваемого узла или None
               "attributes": {
               }
            }
          }
   ...
   * - read
     ...
     - request:
       ...
       .. code:: python

          {
            "action": "read",
            "data": {
               # один или несколько id узлов, по которым необходимо получить
               # данные
               "id": ["<id_1>", "<id_n>"],
               # id узла, с которого начинать поиск
               "base": "<base_id>",
               # флаг разыменования ссылок
               "deref": True,
               # уровень поиска:
               # 1 - сам объект,
               # 2 - один уровень ниже базового узла,
               # 3 - всё дерево ниже базового узла.
               "scope": 3,
               # словарь значений атрибутов, из которых складывается фильтр
               # поиска
               "filter": {
                  "cn": ["some_name_1", "some_name_2"]
               },
               # список атрибутов, значения которых необходимо вернуть
               "attributes": ["description", "prsDefaultValue"]
            }
          }
     ...
     - <сущность>_model_crud
   * - mayUpdate
     - Список ключей маршрутизации указан в иерархии,
       в дочернем узле ``cn=subscribers,cn=system``
     - `+`
     - request:
     ...
     - все сервисы, подписавшиеся на получение уведомлений
       изменения/удаления узлов
   * - updating
     - Список ключей маршрутизации указан в иерархии,
       в дочернем узле ``cn=subscribers,cn=system``
     - `+`
     - request:
       ...
       .. code:: python
          ...
          {
              "response": "ok",
              "message": "some message in case of `no`"
          }
     - все сервисы, подписавшиеся на получение уведомлений
       изменения/удаления узлов
   * - updated
...
Сервис не подписывается ни на какие сообщения, за исключением тех,
которые получает в рамках RPC-вызовов.

.. note:: Планируется посылать сервису команды CRUD также через AMQP,
   а не только через HTTP(S).
