...
.. note:: В таблицах сообщений предполагаем следующее:
   ...
   2. Если в колонке ``reply_to`` указан
      параметр, то сообщение имеет тип ``RPC`` и обязательно имеет
      уникальный параметр ``correlation_id``,
      поэтому его не указываем в таблицах.
...
.. list-table:: Сообщения, генерируемые сервисом ``<сущность>_api_crud``
   ...
   * - create
     ...
     - Сервисом генерируется своя

       собственная уникальная очередь

       и её имя передаётся в качестве

       ключа маршрутизации.

       Этим гарантируется, что ответ

       придёт именно тому экземпляру

       сервиса, который сгенерировал

       сообщение о создании нового узла.
