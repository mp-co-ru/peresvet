...
запуска платформы через `./run_one_app.sh` или `./run_one_app_ssl.sh`: Docker
Compose-файлы, Dockerfile-ы, конфигурация сервисов, исходный код runtime-сервисов,
PDF-документация, скрипты генерации сертификатов и этот README.
...
./run_one_app.sh <имя-сервера>
```

Если `<имя-сервера>` не указано, используется текущее имя хоста. Скрипт обновляет
`NGINX_HOST` в `docker/compose/.cont_one_app.env`, собирает образы и запускает
контейнеры. В продуктовом дистрибутиве скрипты запуска подключают
...
`docker/compose/.cont_one_app.secrets.env`; если файл существует, скрипты запуска
подключат его как дополнительный `--env-file`.

### Запуск HTTPS-варианта

`./run_one_app_ssl.sh` рассчитан на то, что серверный сертификат уже подготовлен
до первого запуска. Имя каталога и имена файлов сертификата должны совпадать с
именем сервера, переданным в скрипт:
...
```bash
./run_one_app_ssl.sh <имя-сервера>
```
