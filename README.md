...
`--ssl false`, то есть запускается HTTP-вариант. Скрипт обновляет `NGINX_HOST` в
`docker/compose/.cont_one_app.env`, собирает образы и запускает контейнеры. В
продуктовом дистрибутиве скрипт запуска подключает
