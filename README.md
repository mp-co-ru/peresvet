...
генерации сертификатов и этот README.
...
Если `--hostname` не указан, используется текущее имя хоста. По умолчанию
`--ssl false`, то есть запускается HTTP-вариант. По умолчанию `--build false`,
то есть существующие локальные образы не пересобираются. Скрипт обновляет
`NGINX_HOST` в `docker/compose/.cont_one_app.env`, собирает отсутствующие образы и
...
Чтобы принудительно пересобрать локальные образы перед запуском:
...
Перед запуском `docker compose up` скрипт проверяет наличие внешних Docker-образов
и скачивает отсутствующие с несколькими повторами. Если образы уже загружены на
сервер вручную и доступ к реестрам не нужен, этот шаг можно пропустить:

```bash
PRS_SKIP_IMAGE_PULL=1 ./run_one_app.sh --hostname <имя-сервера>
```

Если при скачивании образов появляется ошибка вида
`failed to fetch anonymous token` / `auth.docker.io` / `i/o timeout`, проблема
находится на уровне доступа Docker daemon к Docker Hub. Частый случай - Docker
...
Docker/хоста, после чего повторите запуск.
